...
bool explosion[maxFirework];
int numParticle[maxFirework];
float posFirework[maxFirework], rgb[3][maxFirework]; // number of visible points of each firework, position of exposion
...
void drawFirework() {
    ...
    for (int i = 0; i < particles.size() / maxParticle; i++) {
        ...
        for (int j = i * maxParticle; j < (i + 1) * maxParticle; j++) {
            ...
            GLfloat sun_mat_emission[] = { rgb[0][i], rgb[1][i], rgb[2][i], 1.0f };   //定义材质的辐射广颜色，为偏红色
    ...
    for (int i = 0; i < particles.size() / maxParticle; i++) { // i-th firework
        ...
        for (int j = i * maxParticle; j < i * maxParticle + maxParticle; j++) { //j-th particle
            ...
            if (!explosion[i]) {
                ...
                if (particle.alpha > 0)
                    particle.alpha -= 0.003f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
            }
            else {
                particle.posX += particle.speedX;
                particle.posY += particle.speedY;
                ...
                if (particle.alpha > 0)
                    particle.alpha -= 0.0005f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
            }
        ...
        if (numParticle[i] == 0) {
            if (explosion[i]) {  // exposion finished
                ...
                for (int j = i * maxParticle; j < i * maxParticle + maxParticle; j++) {
                    ...
                    float speedZ = static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * 0.0004f - 0.0002f;
                    particle.speedX = speedX; particle.speedY = speedY; particle.speedZ = speedZ;
                    particle.alpha = 1.0f; particle.posY = posFirework[i];
...
void generateParticles(float posX, float posY) {
    for (int j = 0; j < maxParticle; j++) {
        ...
        float speedY = log(static_cast<float>(rand()) / static_cast<float>(RAND_MAX)) / 1.2 * 0.0015 + 0.003f;
        //cout << speedY << '\n';
        if (speedY < 0.0015) 
